...
import crl.android.pdfwriter.utils.UtilsWriteHelper;
...
public class IndirectObject extends Base {
	...
	public int writeToFile(RandomAccessFile fos) throws IOException {
		int bytesWrited = 0;
		
		bytesWrited += UtilsWriteHelper.write(fos, mID.toPDFBytes());
		bytesWrited += UtilsWriteHelper.write(fos, " ".getBytes(Config.DEFAULT_CHARSET));
		
		mContent.clear();
		
		bytesWrited += UtilsWriteHelper.write(fos, mContent.beginKeywordBytes());
		
		// j-a-s-d: this can be performed in inherited classes DictionaryObject and StreamObject
		if (mDictionaryContent.hasContent()) {
			bytesWrited += UtilsWriteHelper.write(fos, mDictionaryContent.toPDFBytes());
			if (mStreamContent.hasContent())
				bytesWrited += UtilsWriteHelper.write(fos, mStreamContent.toPDFBytes());
		}
		
		bytesWrited += UtilsWriteHelper.write(fos, mContent.endKeywordBytes());
