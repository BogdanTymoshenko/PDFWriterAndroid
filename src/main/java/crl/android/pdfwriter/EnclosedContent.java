...
public class EnclosedContent extends Base {
	...
	public byte[] toPDFBytes() {
		return UtilsBytes.concat(beginKeywordBytes(), mContent.toString().getBytes(Config.DEFAULT_CHARSET), endKeywordBytes());
	}
	
	public byte[] beginKeywordBytes() {
		return mBegin.getBytes(Config.DEFAULT_CHARSET);
	}
	
	public byte[] endKeywordBytes() {
		return mEnd.getBytes(Config.DEFAULT_CHARSET);
	}
}
